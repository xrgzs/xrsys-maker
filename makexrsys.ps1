...
# Publish image
.\bin\rclone.exe copy "$sysFile.esd" "odb:/Share/Xiaoran Studio/System/Nightly/$sysDate" --progress
if ($?) { Write-Host "Upload Successfully!" } else { Write-Error "Upload Failed!" }
.\bin\rclone.exe copy "$sysFile.json" "odb:/Share/Xiaoran Studio/System/Nightly/$sysDate" --progress
# Set latest
if ($Latest) {
    .\bin\rclone.exe copyto "$sysFile.json" "odb:/Share/Xiaoran Studio/System/Nightly/$sysVer.json" --progress
}